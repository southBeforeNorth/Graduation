...
import rules from './userPassword.rules';
...
export default {
  ...
  methods: {
    submitForm() {
      const that = this;
      this.$refs.userPasswordForm.validate().then(() => {
        const params = {
          oldPassword: this.form.oldPassword,
          newPassword: this.form.newPassword
        };
        userService.changeUserPasswordByOwner(params)
          .then((res) => {
            if (res.success) {
    ...
    resetForm() {
      ...
      this.$refs.userPasswordForm.clearValidate();
    }
  }
};
</script>
